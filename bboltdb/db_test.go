...
func TestDbOperation(t *testing.T) {
	...
	for _, testCase := range testDbCases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			case SAVE:
				err := DbSave(DB_PATH, []byte(BUCKET), []byte(testCase.key), []byte(testCase.value))
				if err != nil {
					t.Errorf("Error saving value: %v", err)
				}
			case LOAD:
				value, err := DbLoad(DB_PATH, []byte(BUCKET), []byte(testCase.key))
				if err != nil {
			...
			case UPDATE:
				err := DbUpdate(DB_PATH, []byte(BUCKET), []byte(testCase.key), []byte(testCase.value))
				if err != nil {
					t.Errorf("Error updating value: %v", err)
				}
			case REMOVE:
				err := DbRemove(DB_PATH, []byte(BUCKET), []byte(testCase.key))
				if err != nil {
	...
	for _, testCase := range testDbCasesWithMap {
		t.Run(testCase.name, func(t *testing.T) {
			...
			case SAVE:
				...
				err = DbSave(DB_PATH, []byte(BUCKET), []byte(testCase.key), jsonValue)
				if err != nil {
					t.Errorf("Error saving value: %v", err)
				}
			case LOAD:
